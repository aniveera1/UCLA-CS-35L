...
scene_t scene;
int nthreads;
float scaled_color[131][131][3];

void *printPixels (void *thread_num) {
  ...
  for( int px=tNum; px<width; px+=nthreads )
    {
	  ...
	  float scaled_color_temp[3];
	  scaled_color_temp[0] = gamma( pixel_color[0] ) * max_color;
	  scaled_color_temp[1] = gamma( pixel_color[1] ) * max_color;
	  scaled_color_temp[2] = gamma( pixel_color[2] ) * max_color;
            
	  /* enforce caps, replace with real gamma */
	  for( int i=0; i<3; i++)
	    scaled_color_temp[i] = max( min(scaled_color_temp[i], 255), 0);
	   ...
	   */
	  for( int i=0; i<3; i++)
	    scaled_color[px][py][i] = scaled_color_temp[i];
        }
    }
}
...
main( int argc, char **argv )
{
  ...
  subsample_dy
    ...
    for (curThread = 0; curThread < nthreads; ++curThread) {
      ret = pthread_join(threadID[curThread], NULL);
        
      /* Check for error */
      if (ret) {
	printf("ERROR: Thread failed to finish");
	exit(1);
      }
    }
